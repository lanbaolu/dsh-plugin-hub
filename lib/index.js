...
import { createSkillsBridge } from './skills-loader.js'
import { createEvalLoop } from './eval-loop.js'
import { createEvalBridge } from './eval-bridge.js'
...
export const DEFAULT_CONFIG = {
  ...
  mcpServers: [],
  /** Skills 生态桥白名单：只加载显式列出的外部 skill 目录，资源标 Experimental。默认空 = 不加载任何 skill。 */
  skillsDirs: [],
  /** 自进化闭环（eval-loop + eval-bridge）总开关：默认关闭，旗舰示例显式开启。 */
  evalLoopEnabled: false,
  /** agent-teams 状态根目录白名单（如 `<workspace>/.agent-teams`）；默认空 = 不轮询。 */
  evalStateDirs: [],
  /** 轮询间隔 ms。 */
  evalPollMs: 30_000,
  /** 验证置信度阈值：低于它不沉淀记忆（防固化坏经验）。 */
  evalMinConfidence: 0.7,
  /** 是否强制要求验证环节。 */
  evalRequireVerification: false,
  /** 是否启用验证适配（接 llm-verifier，实验性 + 需后端配置，默认关闭）。 */
  evalVerifyEnabled: false,
  /** 已处理任务 id 持久化文件；默认 `~/.dsh/plugin-hub/eval-seen.json`。 */
  evalSeenFile: null,
}
...
export function apply(ctx, config = {}) {
  ...
  // ═══ Skills 生态桥（供给面；白名单 = skillsDirs；资源 Experimental，平台不背书）═══
  const skillsBridge = createSkillsBridge({
    skillDirs: config.skillsDirs ?? [],
    provider: 'dsh-plugin-hub',
  })
  {
    const results = skillsBridge.loadAll()
    const failed = results.filter((r) => !r.ok)
    if (failed.length > 0) {
      ctx.logger?.warn?.('[@lanbaolu/dsh-plugin-hub] Skills 桥部分加载失败：%s', failed.map((f) => `${f.name}: ${f.error}`).join('; '))
    }
    const loaded = skillsBridge.resources()
    if (loaded.length > 0) {
      ctx.logger?.info?.('[@lanbaolu/dsh-plugin-hub] Skills 桥已加载 %d 个资源（Experimental）', loaded.length)
    }
  }

  // ═══ 自进化闭环接线（eval-loop + eval-bridge + eval-adapters；默认关闭，旗舰示例显式开启）═══
  // 最小闭环 ①→④：轮询 agent-teams 状态目录 → 新 completed 任务 → onTaskCompleted →
  // 记忆沉淀（eval-adapters：优先 mneme memory_save 工具，回退 jsonl）。
  const evalEnabled = Boolean(config.evalLoopEnabled ?? DEFAULT_CONFIG.evalLoopEnabled)
  let evalLoop = null
  let evalBridge = null
  if (evalEnabled) {
    const adapters = createEvalAdapters(ctx, {
      enableVerify: Boolean(config.evalVerifyEnabled ?? DEFAULT_CONFIG.evalVerifyEnabled),
      log: ctx.logger,
    })
    const evalStateDirs = config.evalStateDirs ?? DEFAULT_CONFIG.evalStateDirs
    evalLoop = createEvalLoop({
      enabled: true,
      minConfidence: config.evalMinConfidence ?? DEFAULT_CONFIG.evalMinConfidence,
      requireVerification: config.evalRequireVerification ?? DEFAULT_CONFIG.evalRequireVerification,
      getTrajectory: config.evalGetTrajectory ?? adapters.getTrajectory,
      verify: config.evalVerify ?? adapters.verify,
      remember: config.evalRemember ?? adapters.remember,
      log: ctx.logger,
    })
    evalBridge = createEvalBridge({
      dirs: evalStateDirs,
      pollMs: config.evalPollMs ?? DEFAULT_CONFIG.evalPollMs,
      onTaskCompleted: (task) => evalLoop.onTaskCompleted(task),
      seenFile: config.evalSeenFile ?? join(homedir(), '.dsh', 'plugin-hub', 'eval-seen.json'),
      log: ctx.logger,
    })
    ctx.effect(() => {
      evalBridge.start()
      return () => evalBridge.stop()
    }, '@lanbaolu/dsh-plugin-hub: eval-loop')
    ctx.logger?.info?.('[@lanbaolu/dsh-plugin-hub] 自进化闭环已开启（状态目录 %d 个，轮询 %dms）', evalStateDirs.length, config.evalPollMs ?? DEFAULT_CONFIG.evalPollMs)
  }
  ...
  const registerIfReady = () => {
    ...
    if (tools && !toolsRegistered) {
      ...
      const baseTools = [
        defineTool({
          name: 'plugin_hub_skills',
          description: '列出 Skills 生态桥已加载的外部 skill 资源（解析自 SKILL.md frontmatter + 指令段 + 附带脚本）。Experimental：桥接内容平台不背书；默认 skillsDirs 为空时不加载任何 skill。',
          parameters: {},
          output: textOutput,
          async execute() {
            return JSON.stringify(skillsBridge.resources().map((r) => ({
              name: r.name,
              description: r.description,
              instructions: r.instructions,
              metadata: r.metadata,
              scripts: r.scripts.map((s) => s.relative),
              references: r.references.map((s) => s.relative),
              warnings: r.warnings,
              experimental: r.experimental,
              platformEndorsement: r.platformEndorsement,
            })), null, 2)
          },
        }),
        defineTool({
          name: 'plugin_hub_eval_status',
          description: '查询自进化闭环（eval-loop + eval-bridge）状态：是否开启、已处理任务数、最近扫描时间、错误。闭环默认关闭，需显式开启。',
          parameters: {},
          output: textOutput,
          async execute() {
            return JSON.stringify({
              enabled: evalEnabled,
              loop: evalLoop ? { isEnabled: evalLoop.isEnabled() } : null,
              bridge: evalBridge ? evalBridge.status() : null,
              note: evalEnabled
                ? '闭环开启中：轮询 agent-teams 状态目录，新 completed 任务 → 记忆沉淀'
                : '闭环默认关闭（evalLoopEnabled: false），不轮询、不沉淀',
            }, null, 2)
          },
        }),
        defineTool({
