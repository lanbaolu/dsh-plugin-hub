...
/** 默认记忆沉淀：追加到 ~/.dsh/plugin-hub/eval-entries.jsonl（零依赖，不耦合 mneme；调用方可注入 evalRemember 接 mneme）。 */
function defaultEvalRemember(entry, opts) {
  const file = join(homedir(), '.dsh', 'plugin-hub', 'eval-entries.jsonl')
...
export function apply(ctx, config = {}) {
  ...
  // ═══ 自进化闭环接线（eval-loop + eval-bridge；默认关闭，旗舰示例显式开启）═══
  // 最小闭环 ①→④：轮询 agent-teams 状态目录 → 新 completed 任务 → onTaskCompleted → 记忆沉淀。
  ...
  if (evalEnabled) {
    ...
    evalLoop = createEvalLoop({
      ...
      getTrajectory: config.evalGetTrajectory,
      verify: config.evalVerify,
      remember: config.evalRemember ?? defaultEvalRemember,
      log: ctx.logger,
    })
