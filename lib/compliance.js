...
/** 定位 verify-plugin 脚本：环境变量 > 仓库内 dsh-plugin-standard > null。 */
export function verifyScriptPath() {
  if (process.env.DSH_PLUGIN_STANDARD_VERIFY) return process.env.DSH_PLUGIN_STANDARD_VERIFY
  const local = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'dsh-plugin-standard', 'scripts', 'verify-plugin.mjs')
 ...
 */
export function runVerify(dir, { timeoutMs = 60_000 } = {}) {
  const script = verifyScriptPath()
  if (!script) {
    return { ok: false, compliant: false, error: '无法定位 verify-plugin（dsh-plugin-standard 未安装）' }
  }
  let r
  try {
    r = spawnSync('node', [script, dir, '--json'], { encoding: 'utf8', timeout: timeoutMs })
  } catch (e) {
